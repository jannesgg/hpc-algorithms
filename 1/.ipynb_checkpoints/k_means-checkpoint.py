...
def generateData(n, plot=None):
    X, y = make_blobs(n_samples=n, n_features=100, cluster_std=1.7, shuffle=False, random_state = 2122)
    if plot:
...
#def batch_centroid(ip, data, start, end, centroids, c, results_queue):
#    cluster_sizes = np.zeros(centroids.shape[0], dtype=int)
#    variation = 0.0
#    for i in range(0,end-start):
#        cluster, dist = nearestCentroid(data[i],centroids)
#        print(i+(end-start)*ip)
#        c[i+(end-start)*ip] = cluster
...
if __name__ == "__main__":
    ...
    start = time.time()
    init_time, total_var, total_ass, total_recomp, c = kmeans(4, 2, X, nr_iter = 10)
    end = time.time()
